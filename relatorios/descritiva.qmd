...
15. Qual a quantidade de ações criminais que apurem crimes contra a vida ou de ameaça contra pessoas ligadas à defesa do meio ambiente ou de movimentos relacionados à proteção de populações indígenas e/ou povos tradicionais que ingressam por ano? Qual o tempo de duração médio dessas ações? Quais as espécies de crime cometidas? Qual a efetividade da identificação da autoria e do cumprimento da pena?
16. Qual a quantidade de ações criminais que apurem crimes relacionados à questão fundiária que ingressam por ano? Qual o tempo de duração médio dessas ações? Quais as espécies de crime cometidas? Qual a efetividade da identificação da autoria e do cumprimento da pena?
...
É importante ressaltar, contudo, que nem sempre é possível responder às perguntas de forma completa. Por ser um tema que está na intersecção de dois tipos processuais, a identificação de casos nem sempre é possível, especialmente considerando somente bases de informações básicas e movimentações processuais, como é o caso do DataJud. Ainda assim, as análises podem servir como guia para investigações mais profundas

## Bases de dados

A análise considera três bases de processos: SireneJud, DataJud e TRF1. As bases de dados são descritas a seguir.

### SireneJud

A base do SireneJud foi fornecida pelo CNJ, a partir de uma extração realizada  foi no dia 26/10/2022. O arquivo foi fornecido no formato CSV, contendo 637.699 linhas e 37 colunas. Diferentes versões da base podem ser acessadas a partir do [link de dados abertos da ferramenta do SireneJud](https://sirenejud.cnj.jus.br/mapa/geral).
...
O primeiro passo da análise foi realizar filtros para obter uma base mais próxima do escopo da pesquisa. Infelizmente, a base do SireneJud não permite a realização de filtros que identifiquem de forma precisa os casos relacionados à lavagem de bens e capitais em crimes ambientais. No entanto, a base auxilia a estudar a dinâmica de crimes ambientais, com diferentes níveis de profundidade.
...
A base de dados do DataJud apresenta uma estrutura similar à do SireneJud, com colunas comuns em bases de dados judiciais, mas agrega todas as informações ao nível de processo. As informações das partes e movimentações são apresentadas de forma agrupada.
...
A base do TRF1 foi obtida via raspagem de dados a partir do banco de decisões do TRF1. Raspagem de dados é um processo de extração de informações de fontes da web, como sites, APIs e arquivos, e armazenamento dos dados coletados em um formato estruturado, como uma planilha ou banco de dados. Esse processo é realizado com o uso de linguagens de programação, que automatizam a extração de dados da web. A raspagem de dados pode ser usada para coletar dados de várias fontes, sendo uma prática comum para acessar dados que são públicos, mas não são abertos.

A vantagem da base do TRF1 é que ela é obtida a partir da consulta de textos nas decisões. Isso é interessante pois permite a consulta de termos relacionados a questões ambientais ao mesmo tempo em que pesquisamos termos relacionados a lavagem de bens e capitais ou corrupção.

Para construir a base, foi necessário partir de algumas palavras-chave envolvendo a temática de crimes ambientais para obter uma lista inicial de processos. A lista foi, então, refinada, para obter informações de processos que se relacionam com corrupção e lavagem de bens e capitais.
...
A base foi, então, filtrada, com o objetivo de listar casos que podem estar dentro do escopo. Aqui existem duas opções de bases: a primeira contém informações de todos os processos envolvendo questões ambientais dentro do recorte temporal e regional da pesquisa, enquanto a segunda contém, além dos recortes anteriores, filtros relacionados à lavagem e corrupção. 

A primeira base contém 10.241 processos. A base foi enriquecida com informações da consulta processual (antiga) do TRF1. Os dados também foram obtidos com raspagem de dados, fornecendo informações como classe, vara, juiz, localização, movimentações e partes. Infelizmente, as partes só estão disponíveis em 2.936 processos.
...
A base grande do TRF1 foi utilizada para responder às questões 2 e 5. Já a base pequena do TRF1 foi utilizada para responder à questão 1.
...
da_cpopg_parsed |> 
  dplyr::filter(purrr::map_int(partes, nrow) > 0)
```


## Análises

A seguir, apresentamos investigações sobre as questões norteadoras. Cada subseção é de uma das questões. As investigações mostram os resultados principais da análise
...
```{r}
da_basicas_amazon <- readr::read_rds("da_basicas_amazon.rds")
```
...
n_estadual <- sum(da_basicas_amazon$trib_esfera == "Estadual")
n_federal <- nrow(da_basicas_amazon) - n_estadual

lab_estadual = glue::glue("{n_estadual} ({scales::percent(n_estadual/nrow(da_basicas_amazon))})")

lab_tribunal_maior <- da_basicas_amazon |> 
...
Do total de `r nrow(da_basicas_amazon)` processos, `r lab_estadual` são estaduais. Dentre os processos estaduais, o tribunal mais comum é o `r lab_tribunal_maior` dos processos estaduais.
...
Os 10 assuntos mais comuns estão na @tbl-assunto. É possível notar que assuntos genéricos como "Dano Ambiental" e "Flora" são bastante presentes nos dados. Por isso, é difícil de medir quais são as atividades que conectam e alimentam a cadeia de fluxos de capitais que promovem o desmatamento apenas com essa informação.
...
da_basicas_amazon |> 
  ...
  dplyr::summarise(
    n = dplyr::n(),
    ilg = round(dplyr::n() / dplyr::first(pop) * 1e5, 2)
  ) |> 
  dplyr::arrange(dplyr::desc(ilg)) |> 
  dplyr::slice_head(n = 10) |> 
  knitr::kable(
...
da_basicas_amazon |> 
  ...
  dplyr::summarise(
    n = dplyr::n(),
    ilg = round(dplyr::n() / dplyr::first(area) * 1000, 2)
  ) |> 
  dplyr::arrange(dplyr::desc(ilg)) |> 
  dplyr::slice_head(n = 10) |> 
  knitr::kable(
...
da_corrup_orgaos |> 
  dplyr::mutate(
    ...
    nome = forcats::fct_lump_n(
      muni_nm, 10, other_level = "Outros"
    ),
    nome = forcats::fct_infreq(muni_nm),
    nome = forcats::fct_other(
      muni_nm, drop = "Outros", other_level = "Outros"
    )
  ) |> 
  dplyr::count(muni_nm) |> 
  dplyr::mutate(prop = formattable::percent(n/sum(n))) |> 
...
A @tbl-tema-trf1 mostra as atividades com maior volume processual na base pequena do TRF1. É possível notar que desmatamento é a atividade que mais aparece na consulta, seguida por garimpo.

```{r}
cjpg_filter_tempo_espaco <- readr::read_rds("../data-raw/trf1/cjpg_filter_tempo_espaco.rds")

cjpg_filter <- readr::read_rds("../data-raw/trf1/cjpg_filter.rds")
...
A @tbl-tipo-pessoa mostra os tipos de pessoa. A maior parte é de pessoas físicas, sendo que quase 2% da base possui autoridades, orgão de representação ou valores vazios.
...
A @tbl-litigantes mostra os quinze maiores litigantes. O nome que mais aparece é o vazio, que são casos em que o campo do CNPJ está vazio. Outros nomes da lista não fazem muito sentido de estarem no polo passivo. Das empresas privadas, destacam-se as empresas de energia.
...
da_partes_rfb |> 
  # dplyr::semi_join(
  #   da_basicas_amazon |> dplyr::filter(trib_esfera == "Estadual"),
  #   "id_processo"
  # ) |> 
  ...
  dplyr::mutate(
    prop = formattable::percent(n_processos/nrow(da_basicas_amazon)),
...
A base do DataJud não disponibiliza nome nem CNPJ das partes, o que impossibilida análises sobre perfil das partes.
...
da_cpopg_parsed |> 
  ...
  # dplyr::semi_join(cjpg_filter, "id_processo") |> 
  tidyr::unnest(partes) |>
...
A análise do SireneJud indica que empresas de energia são as mais comuns em processos de crime ambiental na região. Já a análise do DataJud não foi possível. Por fim, a análise do TRF1 mostra majoritaramente a União e órgãos como IBAMA, INCRA e FUNAI no polo passivo.
...
```{r}
#| label: dispersao-crim-sirenejud
...
survminer::ggsurvplot(
  km_geral, 
  data = da_basicas_amazon, 
  surv.median.line = "hv"
)
```

A @fig-km-assunto mostra a disribuição dos tempos por assunto, considerando os cinco assuntos com maior volume de processos. As curvas são parecidas em forma e apresentam medianas similares.
...
No DataJud, o tempo foi calculado fazendo-se a diferença entre a data de última baixa e a data do primeiro recebimento. O resultado geral é mostrado na é a @fig-km-datajud. Foram retirados tempos maiores de 10 mil dias e tempos menores que zero. A mediana é de aproximadamente 60 meses, ou 5 anos.

```{r}
#| label: fig-km-datajud
#| fig-cap: "Curva de sobrevivência dos tempos de tramitação dos processos"
km_geral <- da_corrup_orgaos |> 
...
survminer::ggsurvplot(
  km_geral, 
  data = da_corrup_orgaos, 
  surv.median.line = "hv"
)
```

A @fig-km-assunto-datajud mostra a disribuição dos tempos por assunto, considerando os cinco assuntos com maior volume de processos. O assunto "Promoção, constituição, financiamento ou integração de Organização Criminosa" apresentou uma mediana de tempo bem menor que os demais, de apenas 21 meses.

```{r}
#| label: fig-km-assunto-datajud
#| fig-cap: "Curva de sobrevivência dos tempos de tramitação dos processos, por assunto"
aux_km_assunto <- da_corrup_orgaos |> 
...
A @fig-km-assunto-trf1 mostra a disribuição dos tempos por assunto, considerando os cinco assuntos com maior volume de processos. Os tempos variam pouco por assunto, sendo que o assunto com menor tempo é "Ambiental" e o assunto com maior tempo é o "Rural Art. 4851".
